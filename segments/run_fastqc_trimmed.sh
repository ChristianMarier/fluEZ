...
# Define script info
SEGMENT_NAME="Trimming FastQC Analysis"
SCRIPT_NAME="run_fastqc_trimmed.sh"
...
# Paths for the trimmed FASTQ files
...
FASTQC_RESULTS_DIR="$PROJECT_DIR/FastQC_trimmed_results"
mkdir -p "$FASTQC_RESULTS_DIR"

# Output file names
HTML_R1="$FASTQC_RESULTS_DIR/${SAMPLE_NAME}_trimmed_R1_fastqc.html"
ZIP_R1="$FASTQC_RESULTS_DIR/${SAMPLE_NAME}_trimmed_R1_fastqc.zip"
HTML_R2="$FASTQC_RESULTS_DIR/${SAMPLE_NAME}_trimmed_R2_fastqc.html"
ZIP_R2="$FASTQC_RESULTS_DIR/${SAMPLE_NAME}_trimmed_R2_fastqc.zip"
...
FASTQC_R1_DIR="$FASTQC_RESULTS_DIR/${SAMPLE_NAME}_trimmed_R1_fastqc"
FASTQC_R2_DIR="$FASTQC_RESULTS_DIR/${SAMPLE_NAME}_trimmed_R2_fastqc"
...
parse_fastqc_summary() {
	...
	echo "$sample_name,$total_sequences,$gc_content,$sequence_length,$mean_q_score" >> "${PROJECT_DIR}/summary/fastqc_trimmed_summary.csv"
}

# Generate summary for R1 and R2
parse_fastqc_summary "$FASTQC_R1_DIR" "${SAMPLE_NAME}_trimmed_R1"
parse_fastqc_summary "$FASTQC_R2_DIR" "${SAMPLE_NAME}_trimmed_R2"

# Output the CSV header if the file is newly created
if [[ ! -s "${PROJECT_DIR}/summary/fastqc_trimmed_summary.csv" ]]; then
	echo "Sample,Sequences Checked,GC Content,Sequence Length,Mean Quality Score" > "${PROJECT_DIR}/summary/fastqc_trimmed_summary.csv"
fi
...
evaluate_fastqc() {
	...
	echo "$sample_name,$qc_status,$adapter_status" >> "${PROJECT_DIR}/summary/fastqc_trimmed_quality_adapters.csv"
}

# Update CSV headers and process summaries for R1 and R2
if [[ ! -s "${PROJECT_DIR}/summary/fastqc_trimmed_quality_adapters.csv" ]]; then
	echo "Sample,QC Status,Adapter Status" > "${PROJECT_DIR}/summary/fastqc_trimmed_quality_adapters.csv"
fi

evaluate_fastqc "$FASTQC_R1_DIR" "${SAMPLE_NAME}_trimmed_R1"
evaluate_fastqc "$FASTQC_R2_DIR" "${SAMPLE_NAME}_trimmed_R2"

...
##
## Infected host mouse RNA-seq differential gene expression using STAR, featureCounts, DESeq2, and custom scripts
##
...
echo "Starting host mouse RNA-seq analysis for sample: $SAMPLE_NAME"
...
bash $CODE_DIR/segments/trim_fastqs_host.sh $PROJECT_DIR $SAMPLE_NAME $THREADS
if [ $? -ne 0 ]; then
...
# Segment 5: Alignment using STAR
echo "Aligning trimmed FASTQ files to reference genome..."
bash $CODE_DIR/segments/align_star.sh $PROJECT_DIR $SAMPLE_NAME $THREADS
if [ $? -ne 0 ]; then
	echo "Alignment failed for $SAMPLE_NAME."
	exit 1
fi

# Segment 6: Quality Control Post-processing
#echo "Performing post-processing quality control..."
#bash $CODE_DIR/segments/qc_post_processing_host.sh $PROJECT_DIR $SAMPLE_NAME $THREADS
#if [ $? -ne 0 ]; then
#	echo "Post-processing QC failed for $SAMPLE_NAME."
#	exit 1
#fi

# Segment 7: Counts Generation
echo "Generating counts..."
bash $CODE_DIR/segments/generate_counts.sh $PROJECT_DIR $SAMPLE_NAME $THREADS
if [ $? -ne 0 ]; then
	echo "Counts generation failed for $SAMPLE_NAME."
	exit 1
fi
